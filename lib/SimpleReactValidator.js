/*
 * This is custom version of SimpleReactValidator (https://github.com/dockwa/simple-react-validator)
 *
 */
...
class SimpleReactValidator {
  constructor(options = {}) {
    ...
    this.rules = {
      ...
      email                : {message: t('validates.email'),                         rule: val => this.helpers.testRegex(val,/^[A-Z0-9.!#$%&'*+-/=?^_`{|}~]+@[A-Z0-9.-]+\.[A-Z]{2,}$/i)},
      ...
      required             : {message: t('validates.required'),                                     rule: val => !this.helpers.isBlank(val), required: true },
      ...
      id_card: {
        message: t('validates.your_id_card_incorrect'),
        rule: (val) => {
          if(val.length < 2) {
            return false
          }

          const idCardLength = 13
          const idCardArray = val.split('')
          let findIdCardArray = val.split('')

          // 
          for(let i = 0; i < findIdCardArray.length - 1; i++) {
            findIdCardArray[i] *= idCardLength - i
          }
          ...
          // sum array
          const sum = findIdCardArray.reduce((a, b) => {
            return a + b
          })

          // mod with 11
          let idCardChecker = sum % 11

          // minus with 11
          idCardChecker = 11 - idCardChecker

          // use the last position of number from 'idCardChecker' to check with the last position of 'findIdCardArray'
          if(idCardChecker == idCardArray[idCardArray.length - 1]) {
            return true
          }
          return false
        },
    ...
    for (let validation of validations) {
      ...
      if (!this.helpers.passes(rule, value, params, rules)) {
          ...
          if(process.browser) {
            ...
            if(el.length > 0) {
              if(el[0].tagName == "INPUT") {
                el[0].classList.add('data-invalid')
              } else {
                // fix for case input is react datepicker //
                if(el[0].children.length > 0) {
                  el[0].children[0].classList.add('data-invalid')
                }
              }
            } 
          }
        ...
        if(process.browser) {
          ...
          if(el.length > 0) {
            if(el[0].tagName == "INPUT") {
              if (el[0].classList.contains('data-invalid')) {
                el[0].classList.remove('data-invalid')
              }
            } else {
              // fix for case input is react datepicker //
              if(el[0].children.length > 0) {
                if (el[0].children[0].classList.contains('data-invalid')) {
                  el[0].children[0].classList.remove('data-invalid')
                }
              }
            }
          }
